from aws_cdk import Stack, aws_ec2 as ec2, aws_rds as rds, RemovalPolicy, CfnOutput
...
class TicketFlowInfraStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        ec2_sg = ec2.SecurityGroup(
            self, "EC2SecurityGroup", vpc=vpc, description="Allow SSH and Web traffic"
        )
        rds_sg = ec2.SecurityGroup(
            self, "RDSSecurityGroup", vpc=vpc, description="Allow Postgres traffic"
        )
        ...
        ec2_sg.add_ingress_rule(
            ec2.Peer.any_ipv4(), ec2.Port.tcp(8000), "Allow Auth Service HTTP traffic"
        )
        ec2_sg.add_ingress_rule(
            ec2.Peer.any_ipv4(),
            ec2.Port.tcp(8001),
            "Allow Booking Service HTTP traffic",
        )

        # Permitir que la EC2 se conecte a la Base de Datos RDS
        rds_sg.add_ingress_rule(
            ec2_sg, ec2.Port.tcp(5432), "Allow EC2 to connect to Postgres"
        )
