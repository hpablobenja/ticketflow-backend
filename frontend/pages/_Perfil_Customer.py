...
try:
    ...
    if res.status_code == 200:
        ...
        with tf.form("update_profile"):
            ...
            new_password = tf.text_input(
                "Nueva Contraseña (Dejar vacío para no cambiar)", type="password"
            )
            if tf.form_submit_button("Guardar Cambios"):
                ...
                up_res = requests.put(
                    f"{USER_SERVICE_URL}/users/{user_id}", json=payload, headers=headers
                )
                if up_res.status_code == 200:
                    tf.success("¡Datos actualizados correctamente!")
                    tf.session_state.user_email = new_email
                else:
                    tf.error("Error al actualizar datos.")
        ...
        if tf.button("❌ Eliminar mi cuenta permanentemente"):
            del_res = requests.delete(
                f"{USER_SERVICE_URL}/users/{user_id}", headers=headers
            )
            if del_res.status_code == 200:
                ...
                tf.error("No se pudo procesar la baja.")
