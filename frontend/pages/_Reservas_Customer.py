...
with col1:
    ...
    if tf.button("🔄 Actualizar Catálogo"):
        ...
        except Exception as e: tf.error(f"Error de conexión: {e}")
    ...
    if tf.button("🔒 Bloquear Entrada (10 min)"):
        if event_id:
            try:
                ...
                if res.status_code == 200: tf.success("🎉 ¡Reserva realizada!")
                else: tf.error(f"Error: {res.json().get('detail', 'No disponible')}")
            except Exception as e: tf.error(f"Error en transacción: {e}")