...
with tf.expander("➕ Crear Nuevo Evento"):
    ...
    tickets = tf.number_input("Inventario Inicial (tickets_left)", min_value=1, value=100)
    if tf.button("Publicar Evento"):
        payload = {"title": title, "tickets_left": tickets}
        res = requests.post(f"{BOOKING_SERVICE_URL}/events/", json=payload, headers=headers)
        if res.status_code == 201: tf.success("Evento creado con éxito.")
    ...
    if res.status_code == 200:
        for ev in res.json():
            with tf.container(border=True):
                ...
                with c1:
                    tf.write(f"**[{ev.get('id')}] {ev.get('title')}** - Disponibles: {ev.get('tickets_left')}")
                with c2:
                    if tf.button("❌ Eliminar", key=f"del_{ev.get('id')}"):
                        d_res = requests.delete(f"{BOOKING_SERVICE_URL}/events/{ev.get('id')}", headers=headers)
                        if d_res.status_code == 200: tf.rerun()
except Exception as e: tf.error(f"Error: {e}")