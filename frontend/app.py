...
tf.set_page_config(
    page_title="TicketFlow - Autenticación", page_icon="🎫", layout="centered"
)

# Inicialización de estados de sesión comunes
if "token" not in tf.session_state:
    tf.session_state.token = None
if "user_id" not in tf.session_state:
    tf.session_state.user_id = None
if "user_email" not in tf.session_state:
    tf.session_state.user_email = None
if "role" not in tf.session_state:
    tf.session_state.role = None
        ...
        if tf.button("Crear Cuenta", use_container_width=True):
            try:
                ...
                if res.status_code == 201:
                    tf.success("¡Usuario creado con éxito! Inicia sesión.")
                else:
                    tf.error("No se pudo completar el registro.")
            except Exception as e:
                tf.error(f"Error de red: {e}")
else:
    tf.success(
        f"Sesión activa como **{tf.session_state.user_email}** ({tf.session_state.role})"
    )
